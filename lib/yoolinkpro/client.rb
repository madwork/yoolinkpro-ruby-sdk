module Yoolinkpro
  class Client
    ...
    # :symbol_keys (true|false|nil) - convert hash keys to symbols
    # :ascii_only (true|false|nil) - encode all high-bit characters as escaped sequences if true
    # :load (:object|:strict|:compat|:null) - and dump mode to use for JSON :strict raises an exception when a non-supported Object is encountered. :compat attempts to extract variable values from an Object using to_json() or to_hash() then it walks the Object's variables if neither is found. The :object mode ignores to_hash() and to_json() methods and encodes variables using code internal to the Oj gem. The :null mode ignores non-supported Objects and replaces them with a null.
    # :time (:unix|:xmlschema|:ruby) - format when dumping in :compat mode :unix decimal number denoting the number of seconds since 1/1/1970, :xmlschema date-time format taken from XML Schema as a String, :ruby Time.to_s formatted String
    # :create_id (String|nil) - create id for json compatible object encoding
    # :max_stack (Fixnum|nil) - maximum size to allocate on the stack for a JSON String
